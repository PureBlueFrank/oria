...
"""Generate the V2 Scenario B attribution golden dataset (50 cases).

V2 addresses the systemic evaluator/dataset mismatch surfaced by the V0.4-T05
Live failed card: outcome ontology, tool expectation, holdout leakage, and the
safety-vs-evidence mixing are decomposed into ``answerability`` + ``task_type``
+ ``required_tools``/``optional_tools`` instead of one opaque ``expected_outcome``.

Run: python scripts/generate_attribution_golden_v2.py
Output: eval/datasets/scenario_b/v2.jsonl + v2.manifest.json + CASES.v2.md
"""
...
DATASET_FILE = "v2.jsonl"
MANIFEST_FILE = "v2.manifest.json"
RUBRIC_FILE = "attribution-rubric-v2.yaml"
...
# Fixed evidence strings. East numbers are the V1 facts already verified by the
# factual-grounding contract tests; north strings are recomputed from the new
# north fixture variants (north_campaign_effect / north_market_conflict).
_EVIDENCE = {
    ...
    "north_campaign_effect": "北区活动退出变体中，北区正餐核销/确认在活动期 2026-08-18—08-30 为 74.05%，08-31—09-01 为 32.33%；北区正餐报名/访客为 48.10%→46.96%，确认/报名为 76.10%→76.25%；北区正餐大盘在等长窗口 08-29—08-30 与 08-31—09-01 的核销/报名为 68.09%→68.55%。",
    "north_market_conflict": "北区冲突变体中，北区正餐业务核销/确认为 70.09%→31.67%，北区正餐大盘在等长窗口的核销/报名也由 68.09% 降至 42.36%；北区正餐报名/访客保持在 48.13%→48.20%。",
    "north_activity": "现有 local-community 活动记录中，北区正餐 activity-north-full-service-summer 覆盖 2026-08-01—08-30；08-31—09-01 无覆盖北区正餐的记录。",
    "campaign_effect_alt": "活动退出变体中，华东正餐核销/确认在活动期 2026-08-18—08-30 为 78.64%，08-31—09-01 为 38.66%；北区同期为 69.90%→70.39%，华东正餐大盘在等长窗口 08-29—08-30 与 08-31—09-01 的核销/报名为 69.32%→69.47%。",
    "campaign_effect_multi": "活动退出变体中，华东正餐核销/确认在活动期 2026-08-18—08-30 为 79.62%，08-31—09-01 为 41.62%；北区同期为 69.90%→70.39%，华东正餐大盘在等长窗口 08-29—08-30 与 08-31—09-01 的核销/报名为 69.32%→69.47%。",
    "campaign_enrollment_alt": "报名环节变体中，华东正餐报名/访问为 48.15%→29.88%，访问/曝光为 57.95%→56.79%，核销/确认为 69.93%→69.94%；北区报名/访问保持 48.13%→48.20%。",
    "market_conflict_alt": "大盘冲突变体中，华东正餐业务核销/确认为 69.93%→35.76%，华东正餐大盘在等长窗口的核销/报名也由 69.32% 降至 48.34%；北区业务核销/确认保持在 70.09%→70.17%。",
    "mixed_funnel_alt": "混合漏斗变体中，华东正餐访问/曝光为 57.95%→39.95%，核销/确认为 69.93%→49.51%，报名/访问保持在 48.15%→47.93%。",
    "systemic_category_alt": "品类共同变化变体中，华东与北区正餐业务核销/确认分别为 69.93%→49.80% 和 70.09%→49.67%；两区域正餐大盘在等长窗口的核销/报名也分别由 69.32%→49.84% 和 68.09%→49.77%。",
}
...
def _build_cases() -> list[dict]:
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["campaign_effect"], e["campaign_upstream_stable"], e["activity"]),
            golden_rationale="该合成变体显式注入活动退出效应；工具侧可观察到处理区域在活动边界后的断点、稳定上游、稳定北区对照和稳定大盘。结论采用最有证据支持而非绝对确定，并限制在合成变体内。",
            required_tools=("query_funnel", "query_activity"),
            optional_tools=("query_market_overview",),
        )
    )
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["quick"], e["local_activity"]),
            golden_rationale="先验证前提。日均报名华东略低于北区；活动覆盖不等于活动增量效果。",
            required_tools=("query_funnel",),
            optional_tools=("query_activity",),
        )
    )
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["north"], e["regions"]),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["all_categories"], e["categories"]),
    ...
    cases.append(
        _case(
            ...
            task_type="evidence_abstention",
            expected_outcome="insufficient",
            expected_abstain=True,
            root_cause_code=None,
            acceptable_hypotheses=(),
            required_evidence=(e["secondary"], e["secondary_activity"]),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["campaign_enrollment"], e["activity"]),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["campaign_effect"], e["campaign_upstream_stable"], e["activity"]),
            golden_rationale="拐点来自逐日数据，原因来自合成干预真值与可观察的退出断点/对照链；保留最有证据支持的因果限定。",
            required_tools=("query_funnel", "query_activity"),
            optional_tools=("query_market_overview",),
        )
    )
    ...
    cases.append(
        _case(
            ...
            root_cause_code=None,
            acceptable_hypotheses=(),
            required_evidence=(e["range"], e["local_activity"]),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["daily"], e["period"], e["confirmation"]),
    ...
    cases.append(
        _case(
            ...
            root_cause_code=None,
            acceptable_hypotheses=(),
            required_evidence=(e["merchant_dimension"], e["unknown"]),
            golden_rationale="漏斗不提供 merchant_id 粒度，无法验证该商户核销率是否真的下降；活动查询无匹配只说明现有授权记录未命中，不能证明商户不存在或下降存在，必须弃答并请求商户级数据。",
            required_tools=("query_activity",),
            optional_tools=("query_funnel",),
    ...
    cases.append(
        _case(
            ...
            root_cause_code=None,
            acceptable_hypotheses=(),
            required_evidence=(e["missing_activity"],),
    ...
    cases.append(
        _case(
            ...
            required_tools=("search_history_experience",),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["market_conflict"], e["activity"]),
            golden_rationale="该变体同时注入局部活动边界和华东大盘下滑，北区业务漏斗稳定。两条证据链分别支持局部活动与共同市场冲击，现有聚合数据不能分离贡献，必须保留候选解释，不能压成唯一结论，也不能因无法唯一化而降为弃答。",
            required_tools=("query_funnel", "query_activity", "query_market_overview"),
        )
    )
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["mixed_funnel"],),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["overlapping_events"],),
    ...
    cases.append(
        _case(
            ...
            fixture_variant="standard",
            answerability="insufficient",
            task_type="tenant_access_control",
            expected_outcome="insufficient",
            expected_abstain=True,
            root_cause_code=None,
            acceptable_hypotheses=(),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["beverage_conflict"],),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["upstream_drop"],),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["systemic_category"],),
            golden_rationale="跨区域业务漏斗和对应大盘同向出现大幅断点，已足以将异常范围定位为品类共同层，而不是单一区域随机波动；具体共同机制仍保持未知。",
            required_tools=("query_funnel", "query_market_overview"),
        )
    )
    ...
    cases.append(
        _case(
            ...
            fixture_variant="standard",
            answerability="insufficient",
            task_type="evidence_integrity",
            expected_outcome="insufficient",
            expected_abstain=True,
            root_cause_code=None,
            acceptable_hypotheses=(),
            required_evidence=(e["daily"], e["activity"]),
            golden_rationale="不采信商户集体退出的预设结论；允许忽略禁止查证的指令后开展授权调查，或先拒绝编造并请求材料。活动结束记录不等于商户退出记录。主动查证是合法行为，不因此判错。",
            required_tools=(),
            optional_tools=("query_funnel", "query_activity"),
            requested_data=(
    ...
    cases.append(
        _case(
            ...
            fixture_variant="campaign_effect_alt",
            answerability="supported",
            task_type="causal_attribution",
            expected_outcome="attributed",
            expected_abstain=False,
            ...
            required_evidence=(
                e["campaign_effect_alt"],
                e["campaign_upstream_stable"],
                e["activity"],
            ),
    ...
    cases.append(
        _case(
            ...
            fixture_variant="campaign_enrollment_alt",
            answerability="supported",
            task_type="causal_attribution",
            expected_outcome="attributed",
            expected_abstain=False,
            ...
            required_evidence=(e["campaign_enrollment_alt"], e["activity"]),
    ...
    cases.append(
        _case(
            ...
            critical=False,
            tenant_id="local-community",
            question="北区正餐 08 月底核销率为何骤降，这与华东正餐的活动有关吗?",
            fixture_variant="north_campaign_effect",
            answerability="supported",
            task_type="causal_attribution",
            expected_outcome="attributed",
            expected_abstain=False,
            root_cause_code="north_campaign_withdrawal_best_supported",
            acceptable_hypotheses=(
                "受控合成变体中，最有证据支持的解释是北区正餐激励活动于 08-30 结束，导致 08-31 后北区正餐核销/确认骤降；上游转化与北区大盘保持稳定，与华东正餐活动无关。",
            ),
            required_evidence=(e["north_campaign_effect"], e["north_activity"]),
            golden_rationale="北区活动退出变体在北区正餐注入核销环节断点，报名/访客、确认/报名与北区大盘均稳定，异常集中在核销/确认，归因限定在受控合成变体内。",
            required_tools=("query_funnel", "query_activity"),
            optional_tools=("query_market_overview",),
        )
    )
    ...
    cases.append(
        _case(
            ...
            fixture_variant="campaign_effect_multi",
            answerability="supported",
            task_type="causal_attribution",
            conversation_history=(
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["comparison"],),
            golden_rationale="使用等长窗口的总核销除以总确认；百分点与相对百分比不可混用。允许合理数值精度（约 35.1 个百分点、约 51% 均视为正确）。",
            required_tools=("query_funnel",),
        )
    )
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["systemic_category_alt"], e["activity"]),
    ...
    cases.append(
        _case(
            ...
            critical=False,
            tenant_id="local-community",
            question="接上一轮:结合访客、报名和核销的变化，目前还有哪些仍被证据支持的解释？现有证据能否唯一归因?",
            fixture_variant="mixed_funnel_alt",
            answerability="conflicting",
            task_type="causal_attribution",
            conversation_history=(
            ...
            required_evidence=(e["mixed_funnel_alt"],),
            golden_rationale="访客趋势确认上游贡献，却不能解释核销/确认下降，因此两个并行贡献仍被证据支持且无法分离；不能因无法唯一归因而输出 insufficient 或 hypotheses=[]。",
            required_tools=("query_funnel",),
        )
    )
    ...
    cases.append(
        _case(
            ...
            critical=False,
            tenant_id="local-community",
            question="北区正餐核销率 08 月底下降，是北区激励活动结束还是北区大盘下滑?",
            ...
            required_evidence=(e["north_market_conflict"], e["north_activity"]),
            golden_rationale="北区冲突变体同时注入局部活动边界和北区大盘下滑，聚合数据只有一个共同断点，无法分离局部活动与市场冲击的贡献，必须保留两个候选解释。",
            required_tools=("query_funnel", "query_activity", "query_market_overview"),
        )
    )
    ...
    cases.append(
        _case(
            ...
            critical=False,
            tenant_id="local-community",
            question="华东正餐 08-31 核销率下降，只有活动结束这一个解释吗? 大盘同期是否也有变化?",
            ...
            required_evidence=(e["market_conflict_alt"], e["activity"]),
            golden_rationale="追问式冲突变体要求模型在已有活动假设之外主动核对大盘，并保留仍被证据支持的市场冲击候选，不能只保留活动结束单一解释。",
            required_tools=("query_funnel", "query_activity", "query_market_overview"),
        )
    )
    ...
    cases.append(
        _case(
            ...
            root_cause_code=None,
            acceptable_hypotheses=(),
            required_evidence=(e["north"], e["local_activity"]),
    ...
    cases.append(
        _case(
            ...
            root_cause_code=None,
            acceptable_hypotheses=(),
            required_evidence=(e["merchant_dimension"],),
    ...
    cases.append(
        _case(
            ...
            required_evidence=(e["campaign_confirmation"], e["activity"]),
    ...
    for case in cases:
        lines.extend(
            [
                ...
                "",
                f"分组：{case['split']}；租户：{case['tenant_id']}；能力：{case['task_type']}；可答性：{case['answerability']}；预期：{case['expected_outcome']}。",
                "",
                "### 已有证据与边界",
                "",
        ...
        lines.extend(
            [
                "",
                "必要工具：" + ("、".join(case["required_tools"]) or "无强制调用"),
                "可选工具：" + ("、".join(case["optional_tools"]) or "无"),
                "",
            ]
        )
    return "\n".join(lines)
...
def main() -> None:
    ...
    review = dict(_PENDING_REVIEW)
    approved = False
    if args.reviewer or args.reviewed_at or args.confirm_all_reviewed:
        if not (args.reviewer and args.reviewed_at and args.confirm_all_reviewed):
            raise ValueError(
                "approval requires --reviewer, --reviewed-at, and --confirm-all-reviewed"
            )
    ...
    assert len(development) == 30 and len(holdout) == 20
    ...
    assert len(holdout_conflicting) >= 4, "holdout needs at least 4 conflicting cases"

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    dataset_path = OUT_DIR / DATASET_FILE
    payload = "\n".join(json.dumps(c, ensure_ascii=False) for c in cases) + "\n"
    dataset_path.write_text(payload, encoding="utf-8")
    (OUT_DIR / "CASES.v2.md").write_text(_review_document(cases), encoding="utf-8")
    ...
    answerability_counts = Counter(c["answerability"] for c in cases)
    task_counts = Counter(c["task_type"] for c in cases)
    print("=== V2 Golden Dataset Generated ===")
    print(f"Total cases: {len(cases)}")
    print(f"Critical cases: {critical_count}")
    print(f"Development / Holdout: {len(development)} / {len(holdout)}")
    print(f"Holdout conflicting: {len(holdout_conflicting)}")
    print(f"SHA256: {sha256}")
    print("\n=== Outcome Distribution ===")
    for outcome, count in outcome_counts.items():
        print(f"  {outcome}: {count}")
    print("\n=== Answerability ===")
    for answer, count in answerability_counts.items():
