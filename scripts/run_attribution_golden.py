"""Run approved Scenario B Golden cases and enforce the frozen Fixture baseline."""
...
async def _run(args: argparse.Namespace) -> int:
    ...
    manifest = root / "eval/datasets/scenario_b/manifest.json"
    rubric = root / "eval/config/attribution-rubric-v1.yaml"
    gates = load_attribution_gates(root / "eval/config/attribution-gates.yaml")
    baseline_path = root / "eval/baselines/attribution/1.json"
    output_path = Path(args.output)
    with tempfile.TemporaryDirectory(prefix="oria-attribution-golden-") as temporary:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default=".artifacts/eval/attribution_v1.json",
