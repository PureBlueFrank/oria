...
V0.1-T10 及后续 Agent 审计修复已完成真实 DeepSeek + 锁定 BGE 双跑，当前口径是“V0.1 Core 与必需 Live 卡均通过”；这不包含企业 Adapter 或其他 Provider。Live 与 Enterprise 测试默认不运行；显式运行时必须同时提供开关和非空已知 target，否则以非零状态拒绝而不是冒充通过。
