...
| 设计 | 重点 |
| --- | --- |
| **契约化插件核心** | `LLMProvider`、Policy、Tool、Retriever、Memory、Guardrail 等 Protocol 通过 Registry/Factory 装配；Mock 与真实实现只换装配，不改核心引擎。不受信扩展的 MCP 隔离边界已纳入架构，交付状态以 [ROADMAP](ROADMAP.md) 为准。 |
| **同一执行层，两种范式** | 同一 LangGraph 执行层同时承载可恢复 Workflow 与有界 ReAct，共享 HITL、Checkpoint、工具协议与治理能力。 |
| **确定性副作用边界** | LLM 做理解、探索、软排序与解释；Policy、状态机、审批、幂等键和 execution ledger 掌控业务写入。 |
| **可回查的检索与评测** | RAG 覆盖混合检索、重排、ACL 与引用校验；冻结数据集、CI 门禁、Live 与人工盲评分层留证，不以 Mock 代替真实能力结论。 |
