...
""",
    ...
    return resolve_runtime_config(
        ...
        environ={
            "ORIA_TEST_PLATFORM_POSTGRES_DSN": _dsn("ORIA_TEST_PLATFORM_POSTGRES_DSN"),
            "ORIA_TEST_BUSINESS_POSTGRES_DSN": _dsn("ORIA_TEST_BUSINESS_POSTGRES_DSN"),
        },
        cwd=tmp_path,
    )
...
async def test_real_postgres_revision_chains_repository_contract_and_rls_reset(
    ...
    async with DatabaseResources(config) as databases:
        ...
        assert await approval_repository.get("tenant-a", "approval-1") == approval
        assert await approval_repository.get("tenant-b", "approval-1") is None
        ...
        assert len(await repository.list_for_eligibility(_ctx("tenant-a"))) == 1
        assert len(await repository.list_for_eligibility(_ctx("tenant-b"))) == 1
        ...
        async with databases.business_sessions() as session:
            await set_session_tenant_context(session, "tenant-a")
            visible = await session.scalar(
                text("SELECT count(*) FROM merchants WHERE tenant_id = :tenant_id"),
                {"tenant_id": "tenant-a"},
            )
...
@pytest.mark.asyncio
async def test_real_official_postgres_saver_through_tenant_adapter(tmp_path: Path) -> None:
    config = _config(tmp_path)
    ...
    async with open_tenant_postgres_saver(config.storage.platform_url) as saver:
        ...
        await saver.aput_writes(saved, (("messages", "value"),), "task", "path")
        loaded = await saver.aget_tuple(saved)
        assert loaded is not None
        assert loaded.config["configurable"]["thread_id"] == "shared"
