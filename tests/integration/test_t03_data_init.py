...
async def test_two_empty_databases_upgrade_seed_and_repeat_idempotently(tmp_path: Path) -> None:
    ...
    assert first.platform_revision == second.platform_revision == "platform_0007"
    assert first.business_revision == second.business_revision == "business_0010"
    ...
    assert _revision(config.data_paths.platform_db, "alembic_version_platform") == "platform_0007"
    assert _revision(config.data_paths.business_db, "alembic_version_business") == "business_0010"
